...
revision: str = "20251106_rename_query"
down_revision: str | Sequence[str] | None = "1f375c83b052"
...
def upgrade() -> None:
    ...
    op.add_column(
        "spaces", sa.Column("organization_id", postgresql.UUID(as_uuid=True), nullable=True)
    )
    ...
    op.create_foreign_key(
        "fk_threads_created_by", "threads", "users", ["created_by"], ["id"], ondelete="SET NULL"
    )
    ...
    op.execute(
        "ALTER INDEX IF EXISTS idx_query_documents_query_id RENAME TO idx_thread_documents_thread_id;"
    )
    op.execute(
        "ALTER INDEX IF EXISTS idx_query_documents_document_id RENAME TO idx_thread_documents_document_id;"
    )
    ...
    op.add_column(
        "threads", sa.Column("organization_id", postgresql.UUID(as_uuid=True), nullable=True)
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "ALTER INDEX IF EXISTS idx_thread_documents_thread_id RENAME TO idx_query_documents_query_id;"
    )
    op.execute(
        "ALTER INDEX IF EXISTS idx_thread_documents_document_id RENAME TO idx_query_documents_document_id;"
    )
    ...
    op.create_foreign_key(
        "queries_created_by_fkey", "threads", "users", ["created_by"], ["id"], ondelete="SET NULL"
    )
